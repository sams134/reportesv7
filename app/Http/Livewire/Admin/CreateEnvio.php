...
class CreateEnvio extends Component
{
    ...
    public function generatePDF()
    {
        ...
        // Usamos updateOrCreate para un solo paso.
        // Ajusta la cláusula de búsqueda si necesitas otro criterio:
        $envioFinal = $this->motor
            ->envioFinal()              // relación HasOne o morphOne
            ->updateOrCreate(
                ['tipo_envio' => 1],    // filtro para “existente”
                $data                   // datos a insertar/actualizar
            );

        // Borra los envíos adicionales anteriores
        $envioFinal->enviosAdicionales()->delete();

        // Recorre las partes y crea los adicionales
        collect($this->partes)
            ->filter()
            ->each(
                fn($parte) => $envioFinal
        ...
        // Redirige al PDF
        return redirect()
            ->route('admin.envioPDF', $this->motor)
            ->with('openInNewTab', true);
    }
}
