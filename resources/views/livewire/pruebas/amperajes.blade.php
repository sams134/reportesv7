            ...
            <div class="d-flex justify-content-end my-3">
                ...
                <div class="@if (!$usePT) d-none @else d-flex justify-content-end @endif ">
                    <div class="input-group me-2" style="max-width:160px">
                        <button class="input-group-text" id="basic-addon1" style="cursor: pointer;"
                            wire:click="change_forcePT()">
                            @if ($force_pt)
                        ...
                        <input class="form-control" type="number" placeholder="%desbalance" step="0.1"
                            wire:model="fpt" aria-label="Username" min="0" aria-describedby="basic-addon1" />
                    </div>
                    <div class="input-group me-2" style="max-width:160px">
                        <button class="input-group-text" id="basic-addon1" style="cursor: pointer;"
                            wire:click="change_forceCT()">
                            @if ($force_ct)
                                <i class="fas fa-lock me-2 text-danger"></i>
                            @else
                                <i class="fas fa-lock-open me-2 text-secondary"></i>
                        ...
                        <input class="form-control" type="number" placeholder="%desbalance" step="0.1"
                            wire:model="fct" aria-label="Username" min="0" aria-describedby="basic-addon1" />
                    </div>
                </div>

                <div class="input-group me-2" style="max-width:240px"><span class="input-group-text"
                        id="basic-addon1">% Desbalance</span>
                    <input class="form-control" type="number" placeholder="%desbalance" step="0.1"
                        wire:model="inbalance" aria-label="Username" min="0" aria-describedby="basic-addon1"
                ...
                @else
                    <button type="button" class="btn btn-secondary" disabled> Motor Finalizado
                        {{ auth()->user()->userType }}</button>
                @endif
    ...
    @push('scripts')
        ...
        <script>
            ...
            function initCargaGauge(valor, limitI, limitS) {
                ...
                chart = JSC.chart('chartDiv2', {
                    ...
                    events_ready: function() {
                        console.log('📦 Chart listo, ahora podemos exportar');
                        // Con esto te aseguras de que #chartDiv2 ya contenga el <svg>
                        // Si quieres, podrías guardar aquí una bandera:
            ...
            document.addEventListener('DOMContentLoaded', () => {
                initCargaGauge(
                    {{ number_format(($promA / $amperaje_placa) * 100 * $fct * $fpt, 1) }}, {{ $limit_min }},
                    {{ $limit_max }});
            });
            ...
            function saveGraphNoLoadViaSVG() {
                ...
                // 2) Dentro de ese div, toma el primer <svg>
                const svgEl = innerDiv.getElementsByTagName('svg')[0];
                if (!svgEl) {
                    console.error('SVG no encontrado dentro de', innerDiv);
                    return;
                }

                // 3) Serializa, convierte a Blob y envía (igual que antes)
                ...
                img.onload = () => {
                    ...
                    canvas.toBlob(pngBlob => {
                        ...
                        reader.onloadend = () => {
                            // reader.result ya es un DataURL: "data:image/png;base64,AAA..."
                            ...
                            fetch('/api/save-no-load-chart', {
                                ...
                                .then(async response => {
                                    if (!response.ok) {
                                        ...
                                        console.error('Error HTTP', response.status, '– cuerpo:', text);
                        ...
                        reader.readAsDataURL(pngBlob); // desencadena onloadend
                    }, 'image/png');
                };
                img.src = url;
            }
            ...
            Livewire.on('testExported', function() {
                Swal.fire({
                    icon: 'success',
                    title: '¡Éxito!',
                    text: 'La prueba fue exportada al informe, y se colocó en planilla de pruebas.',
                    timer: 3000,
                    showConfirmButton: false
                });
